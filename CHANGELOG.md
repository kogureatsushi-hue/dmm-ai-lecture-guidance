...
- Word作成時に、同梱の`course-overview-template.docx`をレイアウトの正本として使用するよう変更。
- Artifact Template Pickerを起動せず、テンプレートを読み込めない場合は標準レイアウトへ切り替えず停止するルールを追加。
- Wordテンプレート内のサンプル本文を承認済みMDへ全置換するルールを追加。
- Codex利用手順のサンプルプロンプトをGoogle DriveのURL指定例へ変更。
- PublicリポジトリからSkillを利用するだけの場合、GitHubアカウントが不要であることを明記。
- 利用者側の移行作業：既存利用者はSkillを再インストールまたは更新し、Codexを再起動する。
