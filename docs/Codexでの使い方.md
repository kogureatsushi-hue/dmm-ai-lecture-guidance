...
Skillをインストールして使用するだけなら、GitHubアカウントは必要ありません。
...
CodexでGoogle Driveを利用できる状態にし、次のように依頼します。

~~~text
$course-overview-generator を使って、講師共有用の講座概要を作成してください。

講座資料：＜Google Drive上のPDFファイルURL＞
ワーク使用ファイル：＜Google Driveのワーク使用ファイルフォルダURL＞
講座時間：3時間（休憩10分×2回）
出力先：＜Google Driveの保存先フォルダURL＞

最初にMarkdownで作業プレビューを作成し、私の確認が終わるまでDOCXへ変換しないでください。
不明な演習時間や講座固有の条件は、推測せず質問してください。
~~~

URLは、対象ファイルやフォルダをブラウザで開いたときのURLを指定します。Google Drive連携を使用しない場合は、それぞれをCodexから読み書きできるローカルパスへ置き換えてください。
